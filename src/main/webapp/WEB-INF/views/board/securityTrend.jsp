...
<div class="board_pane" style="background-color:lightgray;font: 12px 맑은 고딕;padding: 10px 0px;">
    ...
    <h1 style="margin: 0px auto 10px auto;width: 200px;text-align: center;">보안동향</h1>

    <div style="text-align: right;">
    <form name="searchForm" action="/securityTrend" method="post">
    ...
    <%  if (bbsList.size() > 0) {
            ...
            <tr>
                ...
                <td style="font-weight: bold;"><a style="color: #3577c8;" href="/securityTrendDtl?page=<%=nPage%>&searchWord=<%=sSearchWord%>&bbs=<%=map.get("bbsId")%>"><%=StringUtil.convertString(map.get("bbsTit")) %></a></td>
...
<% if (bbsList.size() > 0) { %>
    <div style="margin: 5px auto 0px auto;text-align: center;">
        <a class="pre" href="/securityTrend?page=<%=nPrevPageNo%>&searchWord=<%=sSearchWord%>"><img src="/images/btn_page_prev.gif" width="16" height="16" alt="이전"></a>
<% for (int i=nGroupStartPos; i<=nGroupEndPos; i++) { %>
    <% if (nPage == i) { %>
        <strong><span><%=i %></span></strong>
    <% } else { %>
        <a href="/securityTrend?page=<%=i %>&searchWord=<%=sSearchWord%>"><span><%=i %></span></a>
    <% }  %>
<% } %>
        <a class="next" href="/securityTrend?page=<%=nNextPageNo%>&searchWord=<%=sSearchWord%>"><img src="/images/btn_page_next.gif" width="16" height="16" alt="다음"></a>
    </div>
<% } %>

    <div class="tableBtnBox">
        <a href="/securityTrendWrite?page=<%=nPage %>&searchWord=<%=sSearchWord%>">
            글쓰기
        </a>
    </div>
