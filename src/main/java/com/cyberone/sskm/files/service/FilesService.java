package com.cyberone.sskm.files.service;
...
import com.cyberone.sskm.files.dao.FilesDao;
...
public class FilesService {
	...
	public int transferFiles(HttpServletRequest request, int nParent, List<MultipartFile> mpfList, int pId) throws Exception {
        ...
        for (MultipartFile mpf : mpfList) {
            ...
            uploadHome = new File(sHomePath + "/sskm_data/product/");
            if (!uploadHome.exists()) {
            	uploadHome.mkdir();
            }
        	
        	File uploadSub = new File(sHomePath + "/sskm_data/product/" + nParent + "/");
        	...
        	File uploadFile = new File(sHomePath + "/sskm_data/product/" + nParent + "/" + mpf.getOriginalFilename());
