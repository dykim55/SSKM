package com.cyberone.sskm.article.service;
...
import com.cyberone.sskm.article.dao.ArticleDao;
...
public class ArticleService {
	...
	public int updateAppxFile(HttpServletRequest request, String sFileType, List<MultipartFile> mpfList, int refCd) throws Exception {
        ...
        for (MultipartFile mpf : mpfList) {
        	...
        	File uploadHome = new File(sHomePath + "/sskm_data/");
            if (!uploadHome.exists()) {
            	uploadHome.mkdir();
            }
        	
            uploadHome = new File(sHomePath + "/sskm_data/article/");
            if (!uploadHome.exists()) {
            	uploadHome.mkdir();
            }
        	
        	File uploadSub = new File(sHomePath + "/sskm_data/article/" + sFileType + "/");
        	...
        	File uploadFile = new File(sHomePath + "/sskm_data/article/" + sFileType + "/" + mpf.getOriginalFilename());
